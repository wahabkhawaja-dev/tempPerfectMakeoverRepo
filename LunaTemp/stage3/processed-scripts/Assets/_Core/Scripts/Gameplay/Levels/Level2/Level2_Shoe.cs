...
public class Level2_Shoe : LevelData
{
    [Space()]
    [Header("----------------------------------------------------------------------------------")]
    [Space()]
    ...
    [Space()]
    public ZoomPos ZoomStep6C;

    [Space()]
    public GameObject PolishHolder;

    [Space()]
    public AudioClip CapOpenClip_6;
    public AudioClip BrushClip_6;

    [Space()]
    public Animator CapAnimator_6;
    ...
    [Space()]
    public SpriteRenderer ToolStep6SR;
    public Sprite ToolStep6SpritePicked;

    [Space()]
    public Transform ToolStep6Target1;
    public Transform ToolStep6Target2;

    [Space()]
    public SpriteRenderer[] LayersToFade_6;
    ...
    void StartStep6()
    {
        ...
        PolishHolder.transform.DOKill();
        PolishHolder.transform.DOLocalMoveX(0f, 1f).SetDelay(1f);

        Invoke(nameof(CapOpen_6), 2.1f);
    }

    public void CapOpen_6()
    {
        AudioController.instance.PlayAnySfx(1, CapOpenClip_6, 0f);

        CapAnimator_6.enabled = true;

        Vector3 startPos = new Vector3(0.9f, -2.5f, 0f);

        ToolStep6.transform.DOKill();
        ToolStep6.transform.DOMoveX(0f, 1f).SetDelay(.5f).OnComplete(() =>
        {
            ToolStep6.transform.DOKill();
            ToolStep6.transform.DOMove(ToolStep6Target1.position, .5f).OnComplete(() =>
            {
                AudioController.instance.PlayAnySfx(2, BrushClip_6, 0f);

                ToolStep6.transform.DOKill();
                ToolStep6.transform.DOMove(ToolStep6Target2.position, .2f).SetLoops(8, LoopType.Yoyo).SetEase(Ease.InOutQuad).OnComplete(() =>
                {
                    ToolStep6.transform.DOKill();
                    ToolStep6.transform.DORotate(new Vector3(0, 0, 4.5f), .5f);
                    ToolStep6.transform.DOMove(startPos, .5f);

                    PolishHolder.transform.DOKill();
                    PolishHolder.transform.DOMoveX(10f, 1f).SetDelay(.5f);
                    ...
                    DOVirtual.DelayedCall(1.2f, () =>
                    {
                        ...
                        {
                            ToolStep6SR.sprite = ToolStep6SpritePicked;
                        };
                    });
                });
            });
    ...
    public void Step6Done()
    {
        ...
        ToolStep6.transform.DOKill();
        ToolStep6.transform.DOMoveX(10f, 1f).SetDelay(.5f).OnComplete(() => 
        { 
            ToolStep6.gameObject.SetActive(false);
        });

        SetProgressBar();
        CameraController.Instance.MoveCamera(ZoomStep6C.CameraPos, ZoomStep6C.CameraFOV);

        for (int i = 0; i < LayersToFade_6.Length; i++)
        {
            LayersToFade_6[i].DOKill();
            LayersToFade_6[i].DOFade(0, 1f);
        }
