...
namespace ScratchCardAsset
{
    public class ScratchCard : MonoBehaviour
    {
        ...
        [Space()]
        [Space()]
        public bool canSpreadMask = true;

        [Space()]
        [Space()]
        public bool shouldPaintHoles = false;

        [Space()]
        [Space()]
        public bool canRotateTip = false;
        ...
        void Update()
        {
            try
            {
                ...
                if (IsScratching)
                {
                    if (shouldPaintHoles)
                    {
                        ToolTip.transform.position = NormalizeThePositionToRotation(ToolTip.transform.rotation);

                        cardInput.ScratchAtPoint(ToolTip.transform.position);
                    }
        ...
        Vector3 NormalizeThePositionToRotation([Bridge.Ref] Quaternion rotation)
        {
        ...
        void OnScratchHole([Bridge.Ref] Vector2 position)
        {
            //  StartCoroutine(cardRenderer.Filling(position));
            cardRenderer.ScratchHoleAddition(position, 1f);
        }

        void OnScratchLine([Bridge.Ref] Vector2 start, [Bridge.Ref] Vector2 end)
        {
            if (canSpreadMask)
                StartCoroutine(cardRenderer.FillingLine(start, end));
            else
                cardRenderer.ScratchLineAddition(start, end, 1f);

        }

        Vector2 GetScratchPosition([Bridge.Ref] Vector2 position)
        {
        ...
        public void ClearSmoothly(float duration = .5f)
        {
            StartCoroutine(ClearSmoothlyRoutine(duration));
        }

        public IEnumerator ClearSmoothlyRoutine(float duration)
        {
            if (cardRenderer == null || RenderTexture == null)
            {
                Debug.LogWarning("Cannot clear smoothly: cardRenderer or RenderTexture is null.");
                yield break;
            }

            float elapsedTime = 0f;
            Texture2D startTexture = GetScratchTexture();
            RenderTexture tempRT = new RenderTexture(RenderTexture.width, RenderTexture.height, 0, RenderTextureFormat.ARGB32);
            Material fadeMat = new Material(GameManager.instance.restoreEffectShader);
            ...
            while (elapsedTime < duration)
            {
                ...
                fadeMat.SetFloat("_Fade", t);
                Graphics.Blit(startTexture, tempRT, fadeMat);
                Graphics.Blit(tempRT, RenderTexture);

                yield return null;
            }

            // Final precision clear (won’t jerk because texture is already transparent)
            cardRenderer.FillRenderTextureWithColor(new Color(0, 0, 0, 0));
            IsScratched = true;
        }
