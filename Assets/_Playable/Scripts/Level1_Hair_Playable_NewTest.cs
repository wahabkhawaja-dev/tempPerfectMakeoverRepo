...
public class Level1_Hair_Playable_NewTest : LevelData
{
    [Space()]
    [Header("----------------------------------------------------------------------------------")]
    ...
    // PLAYABLE: back from the Fix-It level — this level was only switched off, so re-enter step 5 (now fixed) instead of rebooting.
    void ResumeFromFixIt()
    {
        PlayableFadeCover.Cover();
        UI_Manager.instance.InitializeTools(ToolIcons);
        for (int i = 0; i < 2; i++) UI_Manager.instance.SetProgressBarPos();
        ForceCompleteStep4();
    ...
    IEnumerator Start()
    {
        ...
        // PLAYABLE: the Fix-It level hands control back through ResumeFromFixIt().
        PlayableInnerLevel.ResumeOuter = ResumeFromFixIt;
    ...
    void StartStep5()
    {
        ...
        else if (PlayerPrefs.GetInt("CleanShowerPlaced", 0) == 0 && true)
        {
    ...
    bool IsShowerFixed()
    {
        bool remoteStatus = true;  //Add actual remote config value Here
